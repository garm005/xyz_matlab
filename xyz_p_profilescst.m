...
%
% Para usar esta funcion, el registro de datos debe de ser del tipo *.cst y
% tener las siguientes columnas (11):
% Columnas 1-> id,
%          2-7 -> Fecha y tiempo, 
%          8 -> Coordenadas X,
%          9 -> Coordenadas Y, 
%          10 -> Elevación ortometrica,
%          11 -> Elevacion elipsoidal.
%
%
...
[nom1,pat1] = uigetfile('*.cst','Please, select the data file');
if ischar(nom1) == 0 || ischar(pat1) == 0
        error('No se proporciono ningun archivo!!!');
elseif strcmp(nom1(end-3:end),'cst')
        error('Extension de archivo no reconocida!!!');
else
    ...
    datagps(:,12) = datenum(datagps(:,2),datagps(:,3),datagps(:,4),datagps(:,5),datagps(:,8),datagps(:,7));
    
    %%% Clasificando los perfiles
    while kon ~= tam + 1
        if kon ~= tam                             % Si la linea no es el fin del registro...
            actline = datagps(kon,7);
            nexline = datagps(kon+intt,7);
            
            % Transformando las coordenadas de UTM a geograficas
            [datagps(kon,13),datagps(kon,14)] = UTMIP(datagps(kon,8),datagps(kon,9),16,'N');
    ...
    %%% Generando la matriz que contiene todos los perfiles y se usara para
    % el GE, si la version de matlab no es la 2013, se genera el archivo
    % klm de manera manual
    perfi = cell2mat(per);
    if verLessThan('matlab','8.1.0.604') == 1
        geomatrix = [perfi(:,10) perfi(:,13) perfi(:,14)];
        GE_profile('perfiles',geomatrix);
    else
        kmlwriteline('perfiles.klm',perfi(:,12),perfi(:,13),'Color','y');
    end
...
for o = 1:length(per)
   ...
   if verLessThan('matlab','8.1.0.604') == 1
       ...
       kmlwriteline('perfiles',per{o}(:,12),per{o}(:,13),'Color','y');
   end
   fclose all;
end
return
